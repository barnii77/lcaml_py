...
def ast_related_factory():
    ...
    class ModuleDef:
        import __DEPS.typing.List
        import __DEPS.token_type.Token
...
def lcaml_expression_factory():
    ...
    class ModuleDef:
        ...
        import __DEPS.resolvable.Resolvable
        import __DEPS.gettable.Gettable
        import __DEPS.ast_related.AstRelated
        import __DEPS.lcaml_lexer.Syntax
        import __DEPS.token_type.Token, __DEPS.__DEPS.token_type.TokenKind
        import __DEPS.lcaml_utils.PhantomType, __DEPS.lcaml_utils.split_at_context_end
        import __DEPS.interpreter_types.Object, __DEPS.interpreter_types.DType
        import __DEPS.parser_types.AstIdentifier
        import __DEPS.interpreter_vm.InterpreterVM
        import __DEPS.operation_kind.OperationKind
        import __DEPS.typing.List, __DEPS.typing.Dict, __DEPS.typing.Optional, __DEPS.typing.Set, __DEPS.typing.Iterable
        __DEPS.token_type.TokenStream = __DEPS.typing.List[__DEPS.token_type.Token]
        Context = __DEPS.typing.Dict[AstIdentifier, __DEPS.interpreter_types.Object]
        
        
        SYMBOL_TO_OPKIND = {
            "+": __DEPS.operation_kind.OperationKind.ADD,
            "-": OperationKind.SUB,
            "*": OperationKind.MUL,
            "/": OperationKind.DIV,
            "%": OperationKind.MOD,
        ...
        class Function(AstRelated, Resolvable):
            ...
            def __init__(
                self, body, arguments: __DEPS.typing.List[AstIdentifier], bounds: __DEPS.typing.Iterable[AstIdentifier]
            ):
                self.body = body
                self.arguments = arguments
                if isinstance(bounds, dict):
                    self.bounds: __DEPS.typing.Dict[AstIdentifier, __DEPS.interpreter_types.Object] = bounds
                else:
                    self.bounds: __DEPS.typing.Dict[AstIdentifier, __DEPS.interpreter_types.Object] = {ident: None for ident in bounds}
            ...
            def from_stream(
                cls,
                stream: __DEPS.token_type.TokenStream,
                ...
                if first_token.type != __DEPS.token_type.__DEPS.token_type.TokenKind.FUNCTION_ARGS:
                    raise ValueError(
                ...
                arguments = list(
                    map(
                        lambda raw_id: AstIdentifier(__DEPS.token_type.Token(__DEPS.token_type.__DEPS.token_type.TokenKind.IDENTIFIER, raw_id)),
                        identifiers_raw,
                    )
                )
                # consume function body
                first_body_token = stream.pop(0)
                if first_body_token.type != __DEPS.token_type.__DEPS.token_type.TokenKind.LCURLY:
                    raise ValueError(
                ...
                body_stream, remaining_stream = __DEPS.lcaml_utils.split_at_context_end(
                    stream, __DEPS.token_type.Token(__DEPS.token_type.__DEPS.token_type.TokenKind.RCURLY, __DEPS.lcaml_utils.PhantomType())
                )
        ...
        class Operation(AstRelated, Resolvable):
            def __init__(self, left: __DEPS.typing.Optional[Resolvable], operation: __DEPS.token_type.Token, right: Resolvable):
        ...
        class StructType(AstRelated, Resolvable):
            ...
            def __init__(self, fields: __DEPS.typing.List[AstIdentifier]):
                self.fields = fields
            ...
            @classmethod
            def from_stream(cls, stream: __DEPS.token_type.TokenStream):
                token = stream.pop(0)
                if token.type != __DEPS.token_type.__DEPS.token_type.TokenKind.STRUCT:
                    raise ValueError(f"expected struct keyword, got {token}")
                token = stream.pop(0)
                if token.type != __DEPS.token_type.__DEPS.token_type.TokenKind.LCURLY:
                    raise ValueError(f"expected lcurly, got {token}")
        
                state = StructParseState.ExpectFieldOrEnd
                fields: __DEPS.typing.List[AstIdentifier] = []
                while stream:
                    token = stream.pop(0)
                    if state == StructParseState.ExpectFieldOrEnd:
                        if token.type == __DEPS.token_type.__DEPS.token_type.TokenKind.IDENTIFIER:
                            field = AstIdentifier(token)
                            fields.append(field)
                            state = StructParseState.ExpectCommaOrEnd
                        elif token.type == __DEPS.token_type.__DEPS.token_type.TokenKind.RCURLY:
                            break
                        else:
                            raise ValueError(f"Expected field, got {token}")
                    elif state == StructParseState.ExpectCommaOrEnd:
                        if token.type == __DEPS.token_type.__DEPS.token_type.TokenKind.COMMA:
                            state = StructParseState.ExpectFieldOrEnd
                        elif token.type == __DEPS.token_type.__DEPS.token_type.TokenKind.RCURLY:
                            break
                        else:
        ...
        class StructInstance(AstRelated, Resolvable, Gettable):
            def __init__(
                self, fields: __DEPS.typing.Dict[AstIdentifier, Resolvable], type: __DEPS.typing.Optional[Resolvable] = None
            ):
                self.type = type
                self.fields = fields
        ...
        class FunctionCall(AstRelated, Resolvable):
            ...
            Attributes:
                ...
                arguments: (__DEPS.typing.List[Expression]) arguments to call function with
        
            """
        
            def __init__(self, fuction_resolvable: Resolvable, arguments: __DEPS.typing.List):
                """
                Resolved by spawning a new InterpreterVM
        
                Args:
                    function_container: __DEPS.interpreter_types.Object[Function]
                    arguments: __DEPS.typing.List[Expression]
                """
            ...
            def resolve(self, context: Context) -> __DEPS.typing.Optional[__DEPS.interpreter_types.Object]:
                """
        ...
        class Constant(AstRelated, Resolvable):
            ...
            def __init__(self, token: __DEPS.token_type.Token, syntax: Syntax = Syntax()):
                if token.type == __DEPS.token_type.__DEPS.token_type.TokenKind.UNIT_TYPE:
                    value = None
                    kind = __DEPS.interpreter_types.DType.UNIT
                elif token.type == __DEPS.token_type.__DEPS.token_type.TokenKind.INTEGER:
                    try:
                        value = int(token.value)
                        kind = __DEPS.interpreter_types.DType.INT
                    except ValueError:
                        raise ValueError(f"Invalid integer {token.value}")
                elif token.type == __DEPS.token_type.__DEPS.token_type.TokenKind.FLOATING_POINT:
                    try:
                        value = float(token.value)
                        kind = __DEPS.interpreter_types.DType.FLOAT
                    except ValueError:
                        raise ValueError(f"Invalid floating point {token.value}")
                elif token.type == __DEPS.token_type.__DEPS.token_type.TokenKind.STRING_LITERAL:
                    value = token.value
                    kind = __DEPS.interpreter_types.DType.STRING
                elif token.type == __DEPS.token_type.__DEPS.token_type.TokenKind.BOOLEAN:
                    try:
        ...
        class Expression(AstRelated, Resolvable):
            ...
            Attributes:
                expression: __DEPS.token_type.TokenStream of expression
            ...
            @classmethod
            def _build_from(cls, stream: __DEPS.token_type.TokenStream, syntax: Syntax = Syntax()):
                """
                ...
                """
                all_symbols_used: __DEPS.typing.Set[Variable] = set()
                first_pass_buffer = []
                ...
                while stream:
                    ...
                    if token.type == __DEPS.token_type.__DEPS.token_type.TokenKind.OPERATOR:
                        op = Operation(None, token, None)
                        first_pass_buffer.append(op)
                    elif token.type in __DEPS.token_type.__DEPS.token_type.TokenKind._builtin_types:
                        first_pass_buffer.append(Constant(token, syntax))
                    elif token.type == __DEPS.token_type.__DEPS.token_type.TokenKind.IDENTIFIER:
                        first_pass_buffer.append(Variable(AstIdentifier(token)))
                    elif token.type == __DEPS.token_type.__DEPS.token_type.TokenKind.LPAREN:
                        expression, stream, symbols_used = cls.from_stream(
                            stream, syntax, __DEPS.token_type.Token(__DEPS.token_type.__DEPS.token_type.TokenKind.RPAREN, __DEPS.lcaml_utils.PhantomType())
                        )
            ...
            def from_stream(
                ...
                terminating_token: __DEPS.token_type.Token = __DEPS.token_type.Token(
                    __DEPS.token_type.__DEPS.token_type.TokenKind.SEMICOLON, __DEPS.lcaml_utils.PhantomType()
                ),  # use __DEPS.lcaml_utils.PhantomType so it matches anything of that type
            ):
                """
        
                Args:
                    stream: __DEPS.token_type.TokenStream to parse
                ...
                elif stream[0].type == __DEPS.token_type.__DEPS.token_type.TokenKind.FUNCTION_ARGS:
                ...
                elif stream[0].type == __DEPS.token_type.__DEPS.token_type.TokenKind.STRUCT:
...
def lexer_test_factory():
    ...
    class ModuleDef:
        ...
        import __DEPS.lcaml_lexer.Lexer, __DEPS.lcaml_lexer.Syntax, __DEPS.lcaml_lexer.LexError
        import __DEPS.token_type.Token, __DEPS.__DEPS.token_type.TokenKind
        class Test__DEPS.lcaml_lexer.Lexer(unittest.TestCase):
            def setUp(self):
            ...
            def test_let_keyword(self):
                ...
                self.assertEqual(tokens[0].type, __DEPS.token_type.__DEPS.token_type.TokenKind.LET)
            ...
            def test_identifier(self):
                ...
                tokens = lexer()
                self.assertEqual(len(tokens), 1)
                self.assertEqual(tokens[0].type, __DEPS.token_type.__DEPS.token_type.TokenKind.IDENTIFIER)
            ...
            def test_integer(self):
                ...
                tokens = lexer()
                self.assertEqual(len(tokens), 1)
                self.assertEqual(tokens[0].type, __DEPS.token_type.__DEPS.token_type.TokenKind.INTEGER)
            ...
            def test_floating_point(self):
                ...
                tokens = lexer()
                self.assertEqual(len(tokens), 1)
                self.assertEqual(tokens[0].type, __DEPS.token_type.__DEPS.token_type.TokenKind.FLOATING_POINT)
            ...
            def test_string_literal(self):
                ...
                tokens = lexer()
                self.assertEqual(len(tokens), 1)
                self.assertEqual(tokens[0].type, __DEPS.token_type.__DEPS.token_type.TokenKind.STRING_LITERAL)
            ...
            def test_equals(self):
                ...
                tokens = lexer()
                self.assertEqual(len(tokens), 1)
                self.assertEqual(tokens[0].type, __DEPS.token_type.__DEPS.token_type.TokenKind.EQUALS)
            ...
            def test_semicolon(self):
                ...
                tokens = lexer()
                self.assertEqual(len(tokens), 1)
                self.assertEqual(tokens[0].type, __DEPS.token_type.__DEPS.token_type.TokenKind.SEMICOLON)
                self.assertEqual(tokens[0].value, ";")
            ...
            def test_comment(self):
                ...
                tokens = lexer()
                self.assertEqual(len(tokens), 1)
                self.assertEqual(tokens[0].type, __DEPS.token_type.__DEPS.token_type.TokenKind.COMMENT)
            ...
            def test_operator(self):
                ...
                tokens = lexer()
                self.assertEqual(len(tokens), 1)
                self.assertEqual(tokens[0].type, __DEPS.token_type.__DEPS.token_type.TokenKind.OPERATOR)
                self.assertEqual(tokens[0].value, "+")
            ...
            def test_complex_code(self):
                ...
                tokens = lexer()
                expected_tokens = [
                    __DEPS.token_type.Token(__DEPS.token_type.__DEPS.token_type.TokenKind.LET, "let"),
                    __DEPS.token_type.Token(__DEPS.token_type.__DEPS.token_type.TokenKind.IDENTIFIER, "x"),
                    __DEPS.token_type.Token(__DEPS.token_type.__DEPS.token_type.TokenKind.EQUALS, "="),
                    __DEPS.token_type.Token(__DEPS.token_type.__DEPS.token_type.TokenKind.INTEGER, "10"),
                    __DEPS.token_type.Token(__DEPS.token_type.__DEPS.token_type.TokenKind.SEMICOLON, ";"),
                    __DEPS.token_type.Token(__DEPS.token_type.__DEPS.token_type.TokenKind.COMMENT, "-- x y z"),
                    __DEPS.token_type.Token(__DEPS.token_type.__DEPS.token_type.TokenKind.LET, "let"),
                    __DEPS.token_type.Token(__DEPS.token_type.__DEPS.token_type.TokenKind.IDENTIFIER, "y"),
                    __DEPS.token_type.Token(__DEPS.token_type.__DEPS.token_type.TokenKind.EQUALS, "="),
                    __DEPS.token_type.Token(__DEPS.token_type.__DEPS.token_type.TokenKind.INTEGER, "20"),
                    __DEPS.token_type.Token(__DEPS.token_type.__DEPS.token_type.TokenKind.SEMICOLON, ";"),
                    __DEPS.token_type.Token(__DEPS.token_type.__DEPS.token_type.TokenKind.LET, "let"),
                    __DEPS.token_type.Token(__DEPS.token_type.__DEPS.token_type.TokenKind.IDENTIFIER, "z"),
                    __DEPS.token_type.Token(__DEPS.token_type.__DEPS.token_type.TokenKind.EQUALS, "="),
                    __DEPS.token_type.Token(__DEPS.token_type.__DEPS.token_type.TokenKind.IDENTIFIER, "x"),
                    __DEPS.token_type.Token(__DEPS.token_type.__DEPS.token_type.TokenKind.OPERATOR, "+"),
                    __DEPS.token_type.Token(__DEPS.token_type.__DEPS.token_type.TokenKind.IDENTIFIER, "y"),
                    __DEPS.token_type.Token(__DEPS.token_type.__DEPS.token_type.TokenKind.SEMICOLON, ";"),
                ]
...
def interpreter_test_factory():
    ...
    class ModuleDef:
        ...
        import __DEPS.timeit.timeit
        import __DEPS.interpreter.Interpreter
        # def python_run():
        #     def factorial(n):
        #         if n <= 1:
        #             return 1
...
def lcaml_parser_factory():
    ...
    class ModuleDef:
        ...
        import __DEPS.typing.Union, __DEPS.typing.List, __DEPS.typing.Set
        import __DEPS.token_type.Token, __DEPS.__DEPS.token_type.TokenKind
        import __DEPS.ast_related.AstRelated
        import __DEPS.lcaml_lexer.Syntax
        __DEPS.token_type.TokenStream = __DEPS.typing.List[__DEPS.token_type.Token]
        ...
        class AstStatement(AstRelated):
            ...
            def __init__(
                self,
                type: int,
                value: __DEPS.typing.Union[parser_types.AstAssignment, parser_types.AstReturn],
            ):
                self.type = type
                self.value = value
        ...
        class Ast(AstRelated):
            ...
            Attributes:
                statements: __DEPS.typing.List of AstStatement objects with parse function
        
            """
        
            def __init__(self, statements: __DEPS.typing.List):
            ...
            @classmethod
            def from_stream(cls, stream: __DEPS.token_type.TokenStream, syntax: Syntax = Syntax()):
                """
                ...
                identifier = None
                all_symbols_used: __DEPS.typing.Set[lcaml_expression.Variable] = set()
                while stream:
                    token = stream.pop(0)
        
                    if state == ParseState.ExpectStatementOrCommentOrEnd:
                        if token.type == __DEPS.token_type.__DEPS.token_type.TokenKind.COMMENT:
                            continue
                        elif token.type == __DEPS.token_type.__DEPS.token_type.TokenKind.LET:
                            state = ParseState.ExpectIdentfier
                            identifier = None
                        elif token.type == __DEPS.token_type.__DEPS.token_type.TokenKind.RETURN:
                        ...
                        elif token.type == __DEPS.token_type.__DEPS.token_type.TokenKind.IF:
                            ...
                            statements.append(statement)
                        elif token.type == __DEPS.token_type.__DEPS.token_type.TokenKind.SEMICOLON:
                        ...
                        if token.type == __DEPS.token_type.__DEPS.token_type.TokenKind.EQUALS:
                        ...
                        if token.type == __DEPS.token_type.__DEPS.token_type.TokenKind.SEMICOLON:
                            state = ParseState.ExpectStatementOrCommentOrEnd
                        else:
        ...
        class Parser:
            ...
            def __init__(self, stream: __DEPS.token_type.TokenStream, syntax: Syntax):
                self.stream = stream
                self.syntax = syntax
...
def interpreter_vm_factory():
    ...
    class ModuleDef:
        import __DEPS.typing.Optional
        import __DEPS.parser_types.AstIdentifier, __DEPS.parser_types.AstStatementType, __DEPS.parser_types.AstAssignment, __DEPS.parser_types.AstReturn
        import __DEPS.interpreter_types.Object
        class InterpreterVM:
            """
...
def parser_types_factory():
    ...
    class ModuleDef:
        ...
        import __DEPS.typing.List, __DEPS.typing.Any, __DEPS.typing.Tuple, __DEPS.typing.Set
        import __DEPS.lcaml_utils.split_at_context_end, __DEPS.lcaml_utils.PhantomType
        import __DEPS.token_type.Token, __DEPS.__DEPS.token_type.TokenKind
        import __DEPS.ast_related.AstRelated
        import __DEPS.lcaml_lexer.Syntax
        __DEPS.token_type.TokenStream = __DEPS.typing.List[__DEPS.token_type.Token]
        ...
        class AstIdentifier(AstRelated):
            ...
            def __init__(self, name_token: __DEPS.token_type.Token):
                if name_token.type != __DEPS.token_type.__DEPS.token_type.TokenKind.IDENTIFIER:
        ...
        class AstControlFlow(AstRelated):
            """
            Attributes:
                conditions: (__DEPS.typing.List[AstControlFlowBranch]) list of conditions
            """
            ...
            @classmethod
            def from_stream(
                cls, stream: __DEPS.token_type.TokenStream, syntax: Syntax = Syntax()
            ) -> __DEPS.typing.Tuple[__DEPS.typing.Any, __DEPS.token_type.TokenStream, __DEPS.typing.Set[__DEPS.typing.Any]]:
                # __DEPS.typing.Any because AstControlFlow not yet defined
                """
                ...
                all_symbols_used: __DEPS.typing.Set[lcaml_expression.Variable] = set()
                # constants
                STATEMENT_END_TOKEN = __DEPS.token_type.Token(__DEPS.token_type.__DEPS.token_type.TokenKind.SEMICOLON, __DEPS.lcaml_utils.PhantomType())
                BODY_END_TOKEN = __DEPS.token_type.Token(__DEPS.token_type.__DEPS.token_type.TokenKind.RCURLY, __DEPS.lcaml_utils.PhantomType())
                CONDITION_END_TOKEN = __DEPS.token_type.Token(__DEPS.token_type.__DEPS.token_type.TokenKind.RPAREN, __DEPS.lcaml_utils.PhantomType())
                ...
                ELSE_ARTIFICIAL_IF_EXPRESSION, _, _ = lcaml_expression.Expression.from_stream(
                    [
                        __DEPS.token_type.Token(__DEPS.token_type.__DEPS.token_type.TokenKind.BOOLEAN, syntax._true),
                        STATEMENT_END_TOKEN,
                    ],
                    syntax,
                    STATEMENT_END_TOKEN,
                ...
                if stream.pop(0).type != __DEPS.token_type.__DEPS.token_type.TokenKind.LPAREN:  # check/remove LPAREN
                ...
                if stream.pop(0).type != __DEPS.token_type.__DEPS.token_type.TokenKind.LCURLY:  # check/remove LCURLY
                ...
                while stream:
                    ...
                    if token.type != __DEPS.token_type.__DEPS.token_type.TokenKind.ELSE_IF:
                    ...
                    if stream.pop(0).type != __DEPS.token_type.__DEPS.token_type.TokenKind.LCURLY:
                ...
                if token.type != __DEPS.token_type.__DEPS.token_type.TokenKind.ELSE:
                    stream.insert(0, token)
...
def lcaml_utils_factory():
    ...
    class ModuleDef:
        import __DEPS.typing.List
        import __DEPS.__DEPS.token_type.TokenKind, __DEPS.token_type.Token
        __DEPS.token_type.TokenStream = List[__DEPS.token_type.Token]
        ...
        def is_token_pair(token1: __DEPS.token_type.Token, token2: __DEPS.token_type.Token):
            return (
                (token1.type == __DEPS.token_type.__DEPS.token_type.TokenKind.LPAREN and token2.type == __DEPS.token_type.__DEPS.token_type.TokenKind.RPAREN)
                or (token1.type == __DEPS.token_type.__DEPS.token_type.TokenKind.LSQUARE and token2.type == __DEPS.token_type.__DEPS.token_type.TokenKind.RSQUARE)
                or (token1.type == __DEPS.token_type.__DEPS.token_type.TokenKind.LCURLY and token2.type == __DEPS.token_type.__DEPS.token_type.TokenKind.RCURLY)
            )
        
        
        def split_at_context_end(
            stream: __DEPS.token_type.TokenStream, terminating_token: __DEPS.token_type.Token
        ) -> tuple[__DEPS.token_type.TokenStream, __DEPS.token_type.TokenStream]:
            context_stack = []
            terminating_idx = 0
            while terminating_idx < len(stream):
                ...
                if token.type in (
                    __DEPS.token_type.__DEPS.token_type.TokenKind.LPAREN,
                    __DEPS.token_type.__DEPS.token_type.TokenKind.LSQUARE,
                    __DEPS.token_type.__DEPS.token_type.TokenKind.LCURLY,
                ):
...
def extern_python_factory():
    ...
    class ModuleDef:
        ...
        import __DEPS.typing.Dict, __DEPS.typing.List
        import __DEPS.resolvable.Resolvable
        Context = __DEPS.typing.Dict[parser_types.AstIdentifier, interpreter_types.Object]
        ...
        class ExternPython(Resolvable):
            ...
            def execute(
                self, context: Context, args: __DEPS.typing.List[interpreter_types.Object]
        ...
        class Interpreter:
            ...
            def execute(self):
                ...
                for name, value in lcaml_builtins.BUILTINS.items():
                    ...
                    name_ast_id = AstIdentifier(__DEPS.token_type.Token(__DEPS.token_type.__DEPS.token_type.TokenKind.IDENTIFIER, name))
...
def lcaml_lexer_factory():
    ...
    class ModuleDef:
        ...
        import __DEPS.typing.List, __DEPS.typing.Dict, __DEPS.typing.Callable
        import __DEPS.token_type.Token
        ...
        class Syntax:
            ...
            def __init__(self, **kwargs):
                ...
                self.extract_fn_args: __DEPS.typing.Callable = lambda args_str: args_str.strip("|").split(",")
            ...
            def set_custom(self, kwargs: __DEPS.typing.Dict[str, str]):
                """
        ...
        class Lexer:
            ...
            def __call__(self) -> __DEPS.typing.List[Token]:
                """
...
def lcaml_builtins_factory():
    ...
    class ModuleDef:
        ...
        import __DEPS.extern_python.ExternPython
