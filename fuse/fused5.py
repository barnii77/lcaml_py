...
def lcaml_expression_factory():
    ...
    class ModuleDef:
        SYMBOL_TO_OPKIND = {
            ...
            "-": __DEPS.operation_kind.OperationKind.SUB,
            "*": __DEPS.operation_kind.OperationKind.MUL,
            "/": __DEPS.operation_kind.OperationKind.DIV,
            "%": __DEPS.operation_kind.OperationKind.MOD,
            "!": __DEPS.operation_kind.OperationKind.NOT,
            "==": __DEPS.operation_kind.OperationKind.EQ,
            "!=": __DEPS.operation_kind.OperationKind.NEQ,
            "<": __DEPS.operation_kind.OperationKind.LT,
            ">": __DEPS.operation_kind.OperationKind.GT,
            "~": __DEPS.operation_kind.OperationKind.FLIP,
            "<=": __DEPS.operation_kind.OperationKind.LTE,
            ">=": __DEPS.operation_kind.OperationKind.GTE,
            "||": __DEPS.operation_kind.OperationKind.OR,
            "&&": __DEPS.operation_kind.OperationKind.AND,
            "|": __DEPS.operation_kind.OperationKind.BITOR,
            "&": __DEPS.operation_kind.OperationKind.BITAND,
        }
        ...
        class Function(__DEPS.ast_related.AstRelated, __DEPS.resolvable.__DEPS.resolvable.Resolvable):
            """
            ...
            def __init__(
                ...
                if isinstance(bounds, dict):
                    ...
                    self.bounds = {ident: None for ident in bounds}
            ...
            def from_stream(
                cls,
                stream,
                syntax=__DEPS.lcaml_lexer.Syntax(),
            ):
                """
                ...
                identifiers_raw = map(
                    str.strip, syntax._extract_fn_args(first_token.value))
                arguments = list(
                    map(
                        lambda raw_id: AstIdentifier(__DEPS.token_type.Token(
                            __DEPS.token_type.TokenKind.IDENTIFIER, raw_id)),
                        identifiers_raw,
                    )
                )
                ...
                body_stream, remaining_stream = __DEPS.lcaml_utils.split_at_context_end(
                    stream, __DEPS.token_type.Token(
                        __DEPS.token_type.TokenKind.RCURLY, __DEPS.lcaml_utils.PhantomType())
                )
                remaining_stream.pop(0)  # remove RCURLY
                body, symbols_used = __DEPS.lcaml_parser.Ast.from_stream(
                    body_stream, syntax)
                return (
                    cls(
                        body, arguments, map(
                            lambda variable: variable.identifier, symbols_used)
                    ),
                    remaining_stream,
                    symbols_used,
                )

        class Operation(__DEPS.ast_related.AstRelated, __DEPS.resolvable.__DEPS.resolvable.Resolvable):
            def __init__(self, left, operation, right):
                optype = ModuleDef.SYMBOL_TO_OPKIND.get(operation.value)
                if optype is None:
                    raise ValueError(f"Unknown operation {operation.value}")
                self.left = left
                self.operation = optype
                self.right = right
                self.is_unary = optype in [
                    __DEPS.operation_kind.OperationKind.FLIP, __DEPS.operation_kind.OperationKind.NOT]
            ...
            def resolve(self, context):
                ...
                if self.left is None:
                    left = None  # typehint so the lsp doesnt spam me with warnings
                else:
                    left = self.left.resolve(context)

                right = self.right.resolve(context)
                if left is None and self.operation not in __DEPS.operation_kind.OperationKind._unary:
                    raise ValueError("Left operand must not be None")

                if self.operation == __DEPS.operation_kind.OperationKind.ADD:
                    return left.add(right)
                elif self.operation == __DEPS.operation_kind.OperationKind.SUB:
                    return left.sub(right)
                elif self.operation == __DEPS.operation_kind.OperationKind.MUL:
                    return left.mul(right)
                elif self.operation == __DEPS.operation_kind.OperationKind.DIV:
                    return left.div(right)
                elif self.operation == __DEPS.operation_kind.OperationKind.MOD:
                    return left.mod(right)
                elif self.operation == __DEPS.operation_kind.OperationKind.NOT:
                    return right.bool_not()
                elif self.operation == __DEPS.operation_kind.OperationKind.EQ:
                    return left.eq(right)
                elif self.operation == __DEPS.operation_kind.OperationKind.NEQ:
                    return left.neq(right)
                elif self.operation == __DEPS.operation_kind.OperationKind.LT:
                    return left.lt(right)
                elif self.operation == __DEPS.operation_kind.OperationKind.GT:
                    return left.gt(right)
                elif self.operation == __DEPS.operation_kind.OperationKind.FLIP:
                    return right.flip()
                elif self.operation == __DEPS.operation_kind.OperationKind.LTE:
                    return left.lte(right)
                elif self.operation == __DEPS.operation_kind.OperationKind.GTE:
                    return left.gte(right)
                elif self.operation == __DEPS.operation_kind.OperationKind.OR:
                    return left.bool_or(right)
                elif self.operation == __DEPS.operation_kind.OperationKind.AND:
                    return left.bool_and(right)
                elif self.operation == __DEPS.operation_kind.OperationKind.BITOR:
                    return left.bitor(right)
                elif self.operation == __DEPS.operation_kind.OperationKind.BITAND:
                    return left.bitand(right)
                else:
                    raise ValueError(
                        f"Unknown operation type {self.operation}")

        class StructTypeParseState:
            ExpectFieldOrEnd = 0
            ExpectCommaOrEnd = 1

        class StructType(__DEPS.ast_related.AstRelated, __DEPS.resolvable.__DEPS.resolvable.Resolvable):
            """
            ...
            def from_stream(cls, stream):
                ...
                state = ModuleDef.StructTypeParseState.ExpectFieldOrEnd
                fields = []
                while stream:
                    token = stream.pop(0)
                    if state == ModuleDef.StructTypeParseState.ExpectFieldOrEnd:
                        if token.type == __DEPS.token_type.TokenKind.IDENTIFIER:
                            field = __DEPS.parser_types.AstIdentifier(token)
                            fields.append(field)
                            state = ModuleDef.StructTypeParseState.ExpectCommaOrEnd
                        elif token.type == __DEPS.token_type.TokenKind.RCURLY:
                            break
                        else:
                            raise ValueError(f"Expected field, got {token}")
                    elif state == ModuleDef.StructTypeParseState.ExpectCommaOrEnd:
        ...
        class StructInstance(__DEPS.ast_related.AstRelated, __DEPS.resolvable.__DEPS.resolvable.Resolvable, __DEPS.gettable.Gettable):
            def __init__(
            ...
            def get(self, ident):
                ...
                if ident not in self.fields:
                    raise ValueError(
                        f"Field {ident} not found in struct {self}")
                return self.fields[ident]

            @classmethod
            def from_stream(cls, stream, syntax=__DEPS.lcaml_lexer.Syntax()):
                token = stream.pop(0)
                if token.type != __DEPS.token_type.TokenKind.LCURLY:
                    raise ValueError(f"expected lcurly, got {token}")
                ...
                _expression_terminating_token = __DEPS.token_type.Token(__DEPS.lcaml_utils.EqualsAny(
                    __DEPS.token_type.TokenKind.COMMA, __DEPS.token_type.TokenKind.RCURLY), __DEPS.lcaml_utils.PhantomType())
                while stream:
                    token = stream.pop(0)
                    ...
                    if state == StructInstanceParseState.ExpectFieldOrEnd:
                        ...
                        if field is None:
                            raise ValueError(
                                "Interal Error or Syntax Error: Field is None")
                        ...
                        if token.type == __DEPS.token_type.TokenKind.COMMA:
                            ...
                            raise ValueError(
                                f"expected comma or rcurly, got {token}")
                        state = StructInstanceParseState.ExpectFieldOrEnd
        ...
        class FieldAccess(__DEPS.ast_related.AstRelated, __DEPS.resolvable.Resolvable):
            """
            A class that represents a field access to a value. Can be stacked by having a __DEPS.lcaml_expression.FieldAccess as the object field of another FieldAccess
            ...
            def resolve(self, context):
                ...
                # __DEPS.interpreter_types.Object is fine if wrapped value is gettable
                if isinstance(obj, (Gettable, __DEPS.interpreter_types.Object)):
        ...
        class FunctionCall(__DEPS.ast_related.AstRelated, __DEPS.resolvable.Resolvable):
            """

            Attributes:
                function_resolvable: (__DEPS.resolvable.Resolvable[Function]) function to call
                arguments: (List[Expression]) arguments to call function with

            """

            def __init__(self, fuction_resolvable, arguments):
                """
            ...
            def resolve(self, context):
                ...
                elif isinstance(function, __DEPS.extern_python.__DEPS.extern_python.ExternPython):
                    return function.execute(context, resolved_args)

                else:
                    raise TypeError("Cannot call non-function")

        class Variable(__DEPS.ast_related.AstRelated, __DEPS.resolvable.Resolvable):
            ...
            def resolve(self, context):
                ...
                if result is None:
                    raise RuntimeError(
                        f"LCamlNameError: {self.identifier} is undefined")
                return result

        class Constant(__DEPS.ast_related.AstRelated, __DEPS.resolvable.Resolvable):
            """
            ...
            def __init__(self, token, syntax=__DEPS.lcaml_lexer.Syntax()):
                if token.type == __DEPS.token_type.TokenKind.UNIT_TYPE:
                    ...
                    try:
                        ...
                        raise ValueError(
                            f"Invalid floating point {token.value}")
                    ...
                    raise ValueError(
                        f"Invalid type for constant: {token.type}")
        ...
        class Expression(__DEPS.ast_related.AstRelated, __DEPS.resolvable.Resolvable):
            """
            ...
            @classmethod
            def _build_from(cls, stream, syntax=__DEPS.lcaml_lexer.Syntax()):
                """
                ...
                while stream:
                    ...
                    if token.type == __DEPS.token_type.TokenKind.OPERATOR:
                        ...
                        # reuse logic from Operation handling
                        fa = __DEPS.lcaml_expression.FieldAccess(None, None)
                        ...
                        first_pass_buffer.append(
                            Variable(AstIdentifier(token)))
                    elif token.type == __DEPS.token_type.TokenKind.LPAREN:
                        expression, stream, symbols_used = cls.from_stream(
                            stream, syntax, __DEPS.token_type.Token(
                                __DEPS.token_type.TokenKind.RPAREN, __DEPS.lcaml_utils.PhantomType())
                        )
                ...
                FUNCTION_CALL_ALLOWED_TYPES = (Constant, __DEPS.lcaml_expression.Variable, cls)
                second_pass_buffer = []
                ...
                while first_pass_buffer:
                    ...
                    if isinstance(thing, __DEPS.lcaml_expression.Variable):  # add all symbols to all_symbols_used
                ...
                while prev_pass_buffer:
                    ...
                    if isinstance(thing, __DEPS.lcaml_expression.FieldAccess):
                        ...
                        if isinstance(right, __DEPS.lcaml_expression.Variable):
                ...
                while prev_pass_buffer:
                    ...
                    if isinstance(thing, __DEPS.lcaml_expression.Operation):
                        if thing.is_unary:
                            # NOTE: unary operands are always on the right
                            if not prev_pass_buffer:
                                raise ValueError(
                                    "Unary operand must have right operand")
                            right = prev_pass_buffer.pop(0)
            ...
            def from_stream(
                ...
                stream,
                syntax=__DEPS.lcaml_lexer.Syntax(),
                terminating_token=__DEPS.token_type.Token(
                ...
                if not stream:
                    ...
                    struct_type, remaining_stream, symbols_used = StructType.from_stream(
                        stream)
                ...
                if terminating_token not in stream:
                    raise ValueError(
                        f"Expression must end with a {terminating_token}")
                ...
                expression, symbols_used = cls._build_from(
                    expression_stream, syntax)
    ...
    class ModuleDef:
        class TestLexer(unittest.TestCase):
            ...
            def test_let_keyword(self):
                ...
                self.assertEqual(
                    tokens[0].type, __DEPS.token_type.TokenKind.LET)
            ...
            def test_identifier(self):
                ...
                self.assertEqual(len(tokens), 1)
                self.assertEqual(
                    tokens[0].type, __DEPS.token_type.TokenKind.IDENTIFIER)
            ...
            def test_integer(self):
                ...
                self.assertEqual(len(tokens), 1)
                self.assertEqual(
                    tokens[0].type, __DEPS.token_type.TokenKind.INTEGER)
            ...
            def test_floating_point(self):
                ...
                self.assertEqual(len(tokens), 1)
                self.assertEqual(
                    tokens[0].type, __DEPS.token_type.TokenKind.FLOATING_POINT)
                self.assertEqual(tokens[0].value, "123.45")

            def test_string_literal(self):
                lexer = __DEPS.lcaml_lexer.Lexer(
                    '"Hello, World!"', self.syntax)
                tokens = lexer()
                self.assertEqual(len(tokens), 1)
                self.assertEqual(
                    tokens[0].type, __DEPS.token_type.TokenKind.STRING_LITERAL)
            ...
            def test_equals(self):
                ...
                self.assertEqual(len(tokens), 1)
                self.assertEqual(
                    tokens[0].type, __DEPS.token_type.TokenKind.EQUALS)
            ...
            def test_semicolon(self):
                ...
                self.assertEqual(len(tokens), 1)
                self.assertEqual(
                    tokens[0].type, __DEPS.token_type.TokenKind.SEMICOLON)
                self.assertEqual(tokens[0].value, ";")
            ...
            def test_comment(self):
                ...
                self.assertEqual(len(tokens), 1)
                self.assertEqual(
                    tokens[0].type, __DEPS.token_type.TokenKind.COMMENT)
            ...
            def test_operator(self):
                ...
                self.assertEqual(len(tokens), 1)
                self.assertEqual(
                    tokens[0].type, __DEPS.token_type.TokenKind.OPERATOR)
                self.assertEqual(tokens[0].value, "+")
            ...
            def test_complex_code(self):
                ...
                tokens = lexer()
                expected_tokens = [
                    __DEPS.token_type.Token(
                        __DEPS.token_type.TokenKind.LET, "let"),
                    __DEPS.token_type.Token(
                        __DEPS.token_type.TokenKind.IDENTIFIER, "x"),
                    __DEPS.token_type.Token(
                        __DEPS.token_type.TokenKind.EQUALS, "="),
                    __DEPS.token_type.Token(
                        __DEPS.token_type.TokenKind.INTEGER, "10"),
                    __DEPS.token_type.Token(
                        __DEPS.token_type.TokenKind.SEMICOLON, ";"),
                    __DEPS.token_type.Token(
                        __DEPS.token_type.TokenKind.COMMENT, "-- x y z"),
                    __DEPS.token_type.Token(
                        __DEPS.token_type.TokenKind.LET, "let"),
                    __DEPS.token_type.Token(
                        __DEPS.token_type.TokenKind.IDENTIFIER, "y"),
                    __DEPS.token_type.Token(
                        __DEPS.token_type.TokenKind.EQUALS, "="),
                    __DEPS.token_type.Token(
                        __DEPS.token_type.TokenKind.INTEGER, "20"),
                    __DEPS.token_type.Token(
                        __DEPS.token_type.TokenKind.SEMICOLON, ";"),
                    __DEPS.token_type.Token(
                        __DEPS.token_type.TokenKind.LET, "let"),
                    __DEPS.token_type.Token(
                        __DEPS.token_type.TokenKind.IDENTIFIER, "z"),
                    __DEPS.token_type.Token(
                        __DEPS.token_type.TokenKind.EQUALS, "="),
                    __DEPS.token_type.Token(
                        __DEPS.token_type.TokenKind.IDENTIFIER, "x"),
                    __DEPS.token_type.Token(
                        __DEPS.token_type.TokenKind.OPERATOR, "+"),
                    __DEPS.token_type.Token(
                        __DEPS.token_type.TokenKind.IDENTIFIER, "y"),
                    __DEPS.token_type.Token(
                        __DEPS.token_type.TokenKind.SEMICOLON, ";"),
...
def token_type_factory():
    ...
    class ModuleDef:
        ...
        class TokenKind:
            LET = "let"
            # NOTE: because return is a python keyword, this naming is required
...
def lcaml_parser_factory():
    ...
    class ModuleDef:
        ...
        class AstStatement(__DEPS.ast_related.AstRelated):
            """
        ...
        class Ast(__DEPS.ast_related.AstRelated):
            """
                ...
                while stream:
                    ...
                    if state == ParseState.ExpectStatementOrCommentOrEnd:
                        if token.type == __DEPS.token_type.TokenKind.COMMENT:
                            ...
                            all_symbols_used.update(symbols_used)
                            return_statement = __DEPS.parser_types.AstReturn(
                                expression)
                            ...
                            control_flow, stream, symbols_used = __DEPS.parser_types.__DEPS.parser_types.AstControlFlow.from_stream(
                                stream, syntax
                            )
                            all_symbols_used.update(symbols_used)
                            statement = AstStatement(
                        ...
                        if token.type == __DEPS.token_type.TokenKind.IDENTIFIER:
                            identifier = __DEPS.parser_types.AstIdentifier(
                                token)
                        ...
                        assignment = __DEPS.parser_types.AstAssignment(
        ...
        class Parser:
            ...
            def __init__(self, stream, syntax):
                self.stream = stream
                self.syntax = syntax
...
def interpreter_vm_factory():
    ...
    class ModuleDef:
        class InterpreterVM:
            ...
            def __init__(self, ast, variables=None):
            ...
            def execute(self):
                for statement in self.ast.statements:
                    if statement.type == __DEPS.parser_types.AstStatementType.ASSIGNMENT:
                        ...
                        value = assignment.value.resolve(self.variables)
...
def parser_types_factory():
    ...
    class ModuleDef:
        ...
        class AstIdentifier(__DEPS.ast_related.AstRelated):
            """
            ...
            def __init__(self, name_token):
                if name_token.type != __DEPS.token_type.TokenKind.IDENTIFIER:
                    raise ValueError(
                        "__DEPS.token_type.Token must be an identifier")
        ...
        class AstAssignment(__DEPS.ast_related.AstRelated):
            """
        ...
        class AstReturn(__DEPS.ast_related.AstRelated):
            """
        ...
        class AstControlFlowBranch(__DEPS.ast_related.AstRelated):
            """
            ...
            def __str__(self):
                return (
                    "__DEPS.parser_types.AstControlFlowBranch(" + str(self.condition) +
                    ", " + str(self.body) + ")"
                )

        class AstControlFlow(__DEPS.ast_related.AstRelated):
            """
            Attributes:
                conditions: (List[__DEPS.parser_types.AstControlFlowBranch]) list of conditions
            """
            ...
            def __str__(self):
                return "__DEPS.parser_types.AstControlFlow(" + str(self.branches) + ")"

            @classmethod
            def from_stream(
                cls, stream, syntax=__DEPS.lcaml_lexer.Syntax()
            ):
                # Any because __DEPS.parser_types.AstControlFlow not yet defined
                """
                ...
                Returns:
                    __DEPS.parser_types.AstControlFlow: AstControlFlow object built from tokenstream
                ...
                all_symbols_used = set()
                # constants
                STATEMENT_END_TOKEN = __DEPS.token_type.Token(
                    __DEPS.token_type.TokenKind.SEMICOLON, __DEPS.lcaml_utils.PhantomType())
                BODY_END_TOKEN = __DEPS.token_type.Token(
                    __DEPS.token_type.TokenKind.RCURLY, __DEPS.lcaml_utils.PhantomType())
                CONDITION_END_TOKEN = __DEPS.token_type.Token(
                    __DEPS.token_type.TokenKind.RPAREN, __DEPS.lcaml_utils.PhantomType())
                ...
                ELSE_ARTIFICIAL_IF_EXPRESSION, _, _ = __DEPS.lcaml_expression.Expression.from_stream(
                    [
                        __DEPS.token_type.Token(
                            __DEPS.token_type.TokenKind.BOOLEAN, syntax._true),
                        STATEMENT_END_TOKEN,
                    ],
                    syntax,
                    STATEMENT_END_TOKEN,
                ...
                body, stream = __DEPS.lcaml_utils.split_at_context_end(
                    stream, BODY_END_TOKEN)
                stream.pop(0)  # remove RCURLY
                body, symbols_used = __DEPS.lcaml_parser.Ast.from_stream(
                    body, syntax)
                all_symbols_used.update(symbols_used)
                branch = __DEPS.parser_types.AstControlFlowBranch(expression, body)
                ...
                while stream:
                    ...
                    # check/remove LPAREN
                    if stream.pop(0).type != __DEPS.token_type.TokenKind.LPAREN:
                        raise ValueError("Expected LPAREN after if statement")
                    ...
                    if stream.pop(0).type != __DEPS.token_type.TokenKind.LCURLY:
                        raise ValueError(
                            "Expected LCURLY after else if statement")
                ...
                if token.type != __DEPS.token_type.TokenKind.ELSE:
                    stream.insert(0, token)
                    return __DEPS.parser_types.AstControlFlow(branches), stream, all_symbols_used
                ...
                branch = __DEPS.parser_types.AstControlFlowBranch(
                    ELSE_ARTIFICIAL_IF_EXPRESSION, body)
                branches.append(branch)
...
def lcaml_utils_factory():
    ...
    class ModuleDef:
        ...
        def is_token_pair(token1, token2):
            return (
                (token1.type == __DEPS.token_type.TokenKind.LPAREN and token2.type ==
                 __DEPS.token_type.TokenKind.RPAREN)
...
def extern_python_factory():
    ...
    class ModuleDef:
        Context = Dict[__DEPS.parser_types.AstIdentifier,
                       __DEPS.interpreter_types.Object]

        class ExternPython(__DEPS.resolvable.Resolvable):
            """
    ...
    class ModuleDef:
        class Interpreter:
            ...
            def __init__(self, code):
                ...
                self.ast = __DEPS.lcaml_parser.Parser(
            ...
            def execute(self):
                ...
                for name, value in __DEPS.lcaml_builtins.BUILTINS.items():
                    ...
                    name_ast_id = AstIdentifier(__DEPS.token_type.Token(
                        __DEPS.token_type.TokenKind.IDENTIFIER, name))
    ...
    class ModuleDef:
        ...
        class Syntax:
            ...
            def __init__(self, **kwargs):
                ...
                self._extract_fn_args = lambda args_str: args_str.strip(
                    "|").split(",")
                ...
                self.unit_type = r"\(\)"
                # be careful - define this before int so it first checks this
                ...
                self.operator = "|".join(
                    "".join(f"\\{c}" for c in op) for op in operators)
                ...
                for k, pattern_info in self.patterns():
                    if not isinstance(pattern_info, tuple):
                        if not isinstance(pattern_info, str):
                            raise TypeError(
                                f"Invalid pattern_info type: {type(pattern_info)}")
    ...
    class ModuleDef:
        class DType:
            ...
            def name(code):
                if code == DType.INT:
                    ...
                    return "__DEPS.extern_python.ExternPython"
                else:
        ...
        class Object:
            ...
            def flip(self):
                ...
                if return_type is None:
                    raise TypeError(
                        f"Unsupported operation for type {self.type}")
...
def lcaml_builtins_factory():
    ...
    class ModuleDef:
        class Print(__DEPS.extern_python.ExternPython):
        ...
        class PrintLn(__DEPS.extern_python.ExternPython):
            def execute(self, context, args):
        ...
        class Input(__DEPS.extern_python.ExternPython):
            def execute(self, context, args):
                if len(args) != 1:
        ...
        class IsInstance(__DEPS.extern_python.ExternPython):
            def execute(self, context, args):
                if len(args) != 2:
                    raise ValueError(
                        "is_like takes 2 arguments: struct_instance, struct_type")
                ...
                return __DEPS.lcaml_expression.Object(
                    ...
                    set(struct_type.fields) == set(
                        struct_instance.fields.keys()),
                )

            def __str__(self):
                return "IsLike()"

        class IsLike(__DEPS.extern_python.ExternPython):
            def execute(self, context, args):
                if len(args) != 2:
                    raise ValueError(
                        "is_like takes 2 arguments: struct_instance, struct_type")
